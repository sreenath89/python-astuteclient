...
class AstuteShell(object):

    def __init__(self):
        self.auth_plugin = asclient.AuthPlugin()

    def get_base_parser(self):
        parser = argparse.ArgumentParser(
            prog='astute',
            description=__doc__.strip(),
            epilog='See "astute help COMMAND" '
        ...
        parser.add_argument('--version',
                            action='version',
                            version=astuteclient.__version__)

        parser.add_argument('-d', '--debug',
                            default=bool(cliutils.env('ASTUTECLIENT_DEBUG')
                                         ),
                            action='store_true',
                            help='Defaults to env[ASTUTECLIENT_DEBUG].')
        ...
        parser.add_argument('--astute-url', metavar='<ASTUTE_URL>',
                            dest='os_endpoint',
                            default=cliutils.env('ASTUTE_URL'),
                            help=("DEPRECATED, use --os-endpoint instead. "
                                  "Defaults to env[ASTUTE_URL]."))

        parser.add_argument('--astute_url',
                            dest='os_endpoint',
                            help=argparse.SUPPRESS)
        ...
        parser.add_argument('--astute_api_version',
                            help=argparse.SUPPRESS)
    ...
    def parse_args(self, argv):
        ...
        api_version = options.astute_api_version
...
def main(args=None):
    try:
        ...
        print("Stopping Astute Client", file=sys.stderr)
        sys.exit(130)
